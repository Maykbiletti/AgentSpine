...
const output=await runCodexObserver(input(item,{timestamp:"2026-09-25T12:00:00.000Z"}),{modelCall:async request=>{calls++;seen=request;return {stdout:JSON.stringify({status:"proposal",kind:"next-step-correction",next:"Erst die Prüfsumme prüfen.",question:null})};}});assert.equal(calls,1);assert.equal(seen.model,"gpt-6-sol");assert.equal(hash(await readFile(item.transcript)),before);const context=JSON.parse(output.hookSpecificOutput.additionalContext);assert.equal(context.modelProvider,"codex");assert.equal(context.authority,"context-only");assert.equal(context.completionVerified,false);assert.equal(context.sourceDigest,hash(input(item).prompt));assert.equal(context.source.binding.taskId,scope.currentTaskId);assert.equal(context.source.event,input(item).turn_id);assert.equal(context.source.at,"2026-09-25T12:00:00.000Z");
