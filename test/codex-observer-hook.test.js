...
test("Codex observer uses the active host model once and preserves its private source",async t=>{const item=await fixture(t);await writeFile(item.transcript,`${JSON.stringify({type:"event_msg",payload:{type:"user_message",message:"current prompt"}})}\n`,{flag:"a"});const before=hash(await readFile(item.transcript));let calls=0,seen;
...
test("Codex observer rejects a replaced enrolled transcript and preserves replacement bytes",async t=>{const item=await fixture(t),replacement=`${JSON.stringify({type:"session_meta",payload:{id:"session:other"}})}\n`;await rm(item.transcript);await writeFile(item.transcript,replacement);let calls=0;
assert.equal(await runCodexObserver(input(item,{turn_id:"turn:replaced"}),{modelCall:async()=>{calls++;return {stdout:JSON.stringify({status:"none",kind:"not-current-instruction",next:null,question:null})};}}),null);assert.equal(calls,0);assert.equal(await readFile(item.transcript,"utf8"),replacement);
});
