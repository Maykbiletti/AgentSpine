...
test("Codex child is ephemeral, tool-less and cannot inherit an API key",()=>{const args=codexObserverArguments("gpt-6-sol","PRIVATE_ARGV_SENTINEL");for(const flag of ["--ephemeral","--ignore-user-config","--ignore-rules"])assert.ok(args.includes(flag));for(const value of ["features.apps=false","features.hooks=false","features.multi_agent=false","features.plugins=false","features.shell_tool=false","features.unified_exec=false","project_doc_max_bytes=0","tools.view_image=false","tools.web_search=false"])assert.ok(args.includes(value),value);assert.equal(args[args.indexOf("--sandbox")+1],"read-only");assert.equal(args[args.indexOf("--model")+1],"gpt-6-sol");assert.equal(args.at(-1),"-");assert.ok(!args.join("\0").includes("PRIVATE_ARGV_SENTINEL"));assert.deepEqual(codexObserverEnvironment({PATH:"safe",CODEX_HOME:"host-login",OPENAI_API_KEY:"foreign",OpenAI_Api_Key:"foreign",OPENAI_BASE_URL:"foreign",AZURE_OPENAI_API_KEY:"foreign",aZuRe_OpEnAi_ApI_KeY:"foreign",CODEX_API_KEY:"foreign"}),{PATH:"safe",CODEX_HOME:"host-login"});});
