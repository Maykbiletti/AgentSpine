...
test("release check validates every host version and the exact safe package boundary", async () => {
  ...
  const result = await releaseCheck({
    root, tag: "v0.73.0", allowDirty: true, skipPack: false, json: true
  });
  ...
  assert.equal(result.package.packedSize > 0 && result.package.packedSize <= 536 * 1024, true);
  assert.equal(result.package.unpackedSize > 0 && result.package.unpackedSize <= 2393 * 1024, true);
