...
test("release check validates every host version and the exact safe package boundary", async () => {
  ...
  const result = await releaseCheck({
    root, tag: "v0.49.0", allowDirty: true, skipPack: false, json: true
  });
  assert.equal(result.ok, true);
  assert.equal(result.version, "0.49.0");
  assert.equal(result.package.filename, "agent-spine-0.49.0.tgz");
