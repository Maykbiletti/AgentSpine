...
test("release check validates every host version and the exact safe package boundary", async () => {
  ...
  const result = await releaseCheck({
    root, tag: "v0.72.2", allowDirty: true, skipPack: false, json: true
  });
  assert.equal(result.ok, true);
  assert.equal(result.version, "0.72.2");
  assert.equal(result.package.filename, "agent-spine-0.72.2.tgz");
