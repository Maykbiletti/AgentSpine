...
function installMcp(workspace, options) {
  return installCodexMcp({ skillsRoot: join(workspace, "skills"), ...options });
}
...
test("Codex install updates a stable launcher and verifies the loaded reader before existing state", async (t) => {
  ...
  assert.equal(result.nativeSkillInstalled, true);
...
test("Codex install rejects unmanaged registrations, aliases, and symlinked files", async (t) => {
  ...
  await assert.rejects(installMcp(workspace, { codexHome: unmanaged, packageRoot: ROOT }), /unmanaged AgentSpine/);
  ...
  await assert.rejects(installMcp(workspace, { codexHome: quoted, packageRoot: ROOT }), /unmanaged AgentSpine/);
  ...
  await assert.rejects(installMcp(workspace, { codexHome: aliasHome, packageRoot: ROOT }), /symlink/);
  ...
  if (process.platform !== "win32") {
    ...
    const canonicalized = await installMcp(workspace, {
  ...
  await assert.rejects(installMcp(workspace, { codexHome: targetHome, packageRoot: aliasPackage }), /symlink/);
  ...
  await assert.rejects(installMcp(workspace, { codexHome: linkedConfigHome, packageRoot: ROOT }), /regular non-symlink/);
...
test("Codex host-install CLI requires local confirmation and reaches the stable registration path", async (t) => {
  ...
  const args = ["bin/agentspine.js", "host-install", "codex", "--codex-home", codexHome,
    "--skills-root", skillsRoot, "--package-root", ROOT, "--json"];
  ...
  assert.equal(await readFile(result.skill.skillPath, "utf8"),
    await readFile(join(ROOT, "skills", "agent-spine", "SKILL.md"), "utf8"));
});
...
test("Codex install recovers after a staged crash and serializes parallel updates", async (t) => {
  ...
  await assert.rejects(installMcp(workspace, {
  ...
  const [left, right] = await Promise.all([
    installMcp(workspace, { codexHome, packageRoot: ROOT }),
    installMcp(workspace, { codexHome, packageRoot: ROOT })
  ]);
  ...
  await installMcp(workspace, { codexHome, packageRoot: ROOT });
...
test("Codex install refuses tampered registration without touching unrelated state", async (t) => {
  ...
  const installed = await installMcp(workspace, { codexHome, packageRoot: ROOT });
...
test("Codex launcher rejects a stale cache path and an incompatible tool inventory before state access", async (t) => {
  ...
  await writeFile(unknownPath, unknown, "utf8");
  const stale = await installMcp(workspace, { codexHome: staleHome, packageRoot: staleRoot });
  ...
  await Promise.all([
    mkdir(join(incompatibleRoot, "src"), { recursive: true }),
    mkdir(join(incompatibleRoot, "skills", "agent-spine"), { recursive: true }),
    mkdir(incompatibleHome)
  ]);
  await writeFile(join(incompatibleRoot, "package.json"), '{"name":"agent-spine","version":"0.72.7","type":"module"}\n', "utf8");
  await writeFile(join(incompatibleRoot, "src", "version.js"), 'export const VERSION = "0.72.7";\n', "utf8");
  await writeFile(join(incompatibleRoot, "skills", "agent-spine", "SKILL.md"), [
    "---", "name: agent-spine", "description: Synthetic incompatible reader.", "---", "", "Use no authority.", ""
  ].join("\n"), "utf8");
  await writeFile(join(incompatibleRoot, "src", "mcp.js"), [
    ...
    "? { protocolVersion: '2025-06-18', capabilities: { tools: {} }, serverInfo: { name: 'agent-spine', version: '0.72.7' } }",
    ": { tools: [] }; process.stdout.write(JSON.stringify({ jsonrpc: '2.0', id: message.id, result }) + '\\n'); } });",
    ""
  ].join("\n"), "utf8");
  const incompatible = await installMcp(workspace, { codexHome: incompatibleHome, packageRoot: incompatibleRoot });
