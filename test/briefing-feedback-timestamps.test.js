  ...
  test(`briefing feedback round-trips exact source times across ${label}`, async (t) => {
    ...
    for (const preAnswer of [false, true]) {
      ...
      for (const env of [{ CLAUDE_PLUGIN_ROOT: "/synthetic/claude" },
        ...
        const recall = output.message
          ? JSON.parse(output.message.slice(output.message.indexOf("{"), output.message.lastIndexOf("}") + 1))
          : JSON.parse(output.additionalContext).briefing.preAnswerRecall;
        assert.deepEqual(restore(recall.feedbackCandidates), expected);
        if (output.message) assert.ok(Buffer.byteLength(output.message) <= 1200);
      }
    }
