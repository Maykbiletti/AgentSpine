...
- Development only: one source-bound interpretation or an exact-set clarification for two to three natural user-message candidates can survive restart/compaction beside its task. Three-candidate recall is losslessly compacted for the fixed King field. All candidates remain provenance-bearing `model-suggestion` assumptions with `completionVerified:false`; continuation, authority, completion, and semantic acceptance are unchanged. A real new-session model acceptance is still required.
