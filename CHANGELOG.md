...
- Development only: one source-bound interpretation or an exact-set clarification for two to three natural user-message candidates survives restart/compaction. Multi-session ambiguity reopens every registered source and binds provider, session, message and digests per candidate. Recall fits the fixed King field. Candidates remain `model-suggestion` assumptions with `completionVerified:false`; continuation, authority, completion and semantic acceptance are unchanged. A real new-session model acceptance is still required.
