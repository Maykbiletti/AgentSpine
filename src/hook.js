...
async function runHookCore(input, payload, options) {
  ...
  const diagnostics=resolvedSources.diagnostics,catalogPath=await saveCatalog(catalog),sourceWarning=[...new Set([...resolvedSources.unverified.map((item)=>item.message),diagnostics.warning,...(diagnostics.warnings||[]).map((item)=>item.message)].filter(Boolean))].join("\nAgentSpine source warning: ")||null;
