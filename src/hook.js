...
import {ATTENTION_WRITE_EVENTS,boundedId,captureAttentionLifecycle,hookDeliveryId,hostContextLimit,hostFromInput,promptFromInput,renderContext,runtimeScope,selfstarterInput,selfstarterRootSkipped,selfstarterScope,sessionId,startChannelEvent,startSelfstarter,toolResult,toolSucceeded} from "./lib/hook-context.js";
...
import {isPremortemWrite,prepareHookPremortem,recordHookPremortemWrite,recordHookPremortemWriteIntent,verifyHookPremortemWrite} from "./lib/hook-premortem.js";
...
if (event === "UserPromptSubmit") {
const finalized = await finalizeUserPromptSessionTimeline({ root, input, scope, resolvedSources, preflight,
...
await recordHookBriefingUse({ origin: briefingOrigin, root, handoff, now: input.timestamp || new Date() });
}
return;
} catch (error) {
if (hookScanFailureFailsOpen(error)) {
