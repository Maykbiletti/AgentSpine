...
return mutateObserver(root,bindingDigest,(current,state)=>{const a=state.observers.find(item=>item.bindingDigest===session);if(!a)return {status:"unavailable"};if(!current)current={bindingDigest,model:a.model,prompts:[],authority:AUTHORITY};state.observers=[...new Set([current,a,...state.observers])].slice(0,MAX_SOURCES);if(current.prompts.includes(prompt))return {status:"duplicate"};current.model=a.model;current.prompts.push(prompt);current.prompts=current.prompts.slice(-32);
current.updatedAt=asDate(now).toISOString();return {status:"claimed",model:a.model,save:true};});}
