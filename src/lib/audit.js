...
export async function runAudit(root = process.cwd(), { host = null } = {}) {
  ...
  const { premortems, indexes: premortemIndexes, writeIndexes: premortemWriteIndexes,
    crossReferenceIssues: premortemCrossReferenceIssues } = await inspectPremortemAudit(
    before.root, gatewayPolicy);
  ...
  const premortemRecords = premortems.states.flatMap((state) => [state, state.requirement,
    state.artifact, state.firstWrite, state.lastWrite, state.closure, ...state.events].filter(Boolean))
    .concat(premortemIndexes.pointers, premortemIndexes.finalizations, premortemWriteIndexes.nodes);
  const premortemAuthorityIssues = premortemRecords.filter((record) => record.authority !== "context-only");
  const premortemIssues = [...premortems.errors, ...premortemIndexes.errors, ...premortemWriteIndexes.errors]
    .map((item) => `unreadable-state:${item.reason}`)
    .concat(premortemCrossReferenceIssues.map((item) => `index-state-mismatch:${item.reason}`));
  ...
  const gates = [
    ...
    gate(3, "State isolation", [catalogPath, graphPath, attentionPath, learningPath, continuityPath,
      ...
      ...premortems.paths, ...premortems.errors.map((item) => item.path), premortemIndexes.directory,
      ...premortemIndexes.directories, ...premortemIndexes.paths,
      ...premortemIndexes.errors.map((item) => item.path), ...premortemWriteIndexes.directories,
      ...premortemWriteIndexes.paths, ...premortemWriteIndexes.errors.map((item) => item.path),
      ...premortemWriteIndexes.uncertainties.map((item) => item.path),
      ...premortemWriteIndexes.truncations.map((item) => item.path)].filter(Boolean)
  ...
  return {
    ...
    registryPath,
    feedStatePath,
    premortemDiagnostics: {
      directory: premortems.directory,
      paths: [...premortems.paths, ...premortemWriteIndexes.paths],
      states: premortems.states.length,
      artifacts: premortems.states.filter((state) => state.artifact).length,
      closures: premortems.states.filter((state) => state.closure).length,
      errors: structuredClone([...premortems.errors, ...premortemWriteIndexes.errors]),
      truncations: structuredClone([...premortems.truncations, ...premortemWriteIndexes.truncations]),
      writeIndex: { directories: [...premortemWriteIndexes.directories],
        paths: [...premortemWriteIndexes.paths], nodes: premortemWriteIndexes.nodes.length,
        states: premortemWriteIndexes.states,
        attemptedProofs: premortemWriteIndexes.attemptedProofs,
        verifiedProofs: premortemWriteIndexes.verifiedProofs,
        errors: structuredClone(premortemWriteIndexes.errors),
        uncertainties: structuredClone(premortemWriteIndexes.uncertainties),
        truncations: structuredClone(premortemWriteIndexes.truncations), authority: "context-only" },
      index: {
        directory: premortemIndexes.directory,
        directories: [...premortemIndexes.directories],
        paths: [...premortemIndexes.paths],
        pointers: premortemIndexes.pointers.length,
        finalizations: premortemIndexes.finalizations.length,
        tamperedPointers: [...premortemIndexes.tamperedPointers],
        tamperedFinalizations: [...premortemIndexes.tamperedFinalizations],
        errors: structuredClone(premortemIndexes.errors),
        truncations: structuredClone(premortemIndexes.truncations),
        crossReferenceErrors: structuredClone(premortemCrossReferenceIssues),
