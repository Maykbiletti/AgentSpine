...
function authorityViolations(graph, attention, learning, coordination, sharing, trust, registry, feedState) {
  const records = [
    ...graph.edges,
    ...graph.annotations,
    ...graph.entities,
...
export async function runAudit(root = process.cwd()) {
  ...
  const authority = authorityViolations(graph, attention, learning, coordination, sharing, trust, registry, feedState);
