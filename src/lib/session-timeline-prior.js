...
const CONTINUITY_FIELDS = ["host", "entityId", "userId", "tenantId", "projectId", "taskId"];

function digest(value) {
  return createHash("sha256").update(value).digest("hex");
}

function sameContinuationScope(candidate, current) {
...
export function timelineSessionReference(binding) {
  const material = CONTINUITY_FIELDS.concat("sessionId")
    .map((field) => `${field}:${binding[field] || ""}`).join("\0");
  return `session-ref:${digest(material).slice(0, 32)}`;
}

export function priorTimelineSources(state, scoped) {
  return state.sources.filter((source) => sameContinuationScope(source.binding, scoped))
    .sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
}

export function selectPriorTimelineSource(state, scoped, { target, wanted, windowMs }) {
  const eligible = priorTimelineSources(state, scoped);
...
export function priorTimelineHint(state, scoped, sourceDigest) {
  const sources = priorTimelineSources(state, scoped);
  ...
  return {
    ...
    instruction: sources.length
      ? "Use session_timeline_search with includePriorSessions only for a relevant exact-time or two-term question."
      : null,
