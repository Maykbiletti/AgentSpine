...
function call(command,args,cwd,env,input){return new Promise((resolve,reject)=>{const child=execFile(command,args,{cwd,env,windowsHide:true,maxBuffer:262144},(error,stdout)=>error?reject(error):resolve({stdout,args}));child.stdin.on("error",reject);child.stdin.end(input);});}
function callClaude({cwd,model,prompt,environment}){const args=claudeObserverArguments(model);return call("claude",args,cwd,claudeObserverEnvironment(environment),prompt);}
export function codexObserverArguments(model){const off="apps hooks memories multi_agent plugins shell_tool unified_exec workspace_dependencies".split(" ");return ["exec","--ephemeral","--ignore-user-config","--ignore-rules","--sandbox","read-only","--model",model,...off.flatMap(value=>["-c",`features.${value}=false`]),"-c","project_doc_max_bytes=0","-c","tools.view_image=false","-c","tools.web_search=false","-"];}
function callCodex({cwd,model,prompt,environment}){const args=codexObserverArguments(model);return call("codex",args,cwd,codexObserverEnvironment(environment),`${prompt}\nReturn one JSON object matching this schema: ${JSON.stringify(SCHEMA)}`);}
