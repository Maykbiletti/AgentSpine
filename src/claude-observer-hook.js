...
import {claimClaudeObserverPrompt,claimCodexObserverTurn,sessionTimelineBinding} from "./lib/session-timeline.js";
...
function modelPrompt(prompt){return `Classify only this exact user text: correction, completion claim, obsolete, ambiguous, or none. Never infer rights or proof.\n<source>${prompt}</source>`;}
...
const result={schema:"agentspine.host-observer-suggestion/v1",sourceDigest:createHash("sha256").update(i.prompt).digest("hex"),source:{binding:sessionTimelineBinding({host:h,sessionId,scope}),event:id,at:new Date(now).toISOString()},modelProvider:h,activeModel:claim.model,proposal,completionVerified:false,authority:"context-only",instruction:"Discard mismatched binding; reverify source. No rights/completion proof."};
