...
function validResult(value){if(!value||typeof value!=="object"||Object.keys(value).sort().join()!=="kind,next,question,status"||!["none","proposal"].includes(value.status)||!KINDS.includes(value.kind))return false;const {next,question}=value;if(next!==null&&(typeof next!=="string"||next.length>500)||question!==null&&(typeof question!=="string"||question.length>300))return false;if(value.status==="none")return next===null&&question===null;if(value.kind==="next-step-correction")return !!next?.trim()&&!/[\r\n]/u.test(next)&&question===null;return next===null&&(value.kind==="ambiguous"?!!question?.trim()&&!/[\r\n]/u.test(question):question===null);}
...
function modelPrompt(prompt){return `Classify this exact user text: task correction, completion claim, obsolete, ambiguous or none. No rights or completion proof.\n<source>${prompt}</source>`;}
