...
function call(command,args,cwd,env){return new Promise((resolve,reject)=>{const child=execFile(command,args,{cwd,env,windowsHide:true,maxBuffer:262144},(error,stdout)=>error?reject(error):resolve({stdout,args}));child.stdin.end();});}
...
export function codexObserverArguments(model,prompt){const off="apps hooks memories multi_agent plugins shell_tool unified_exec workspace_dependencies view_image".split(" ");return ["exec","--ephemeral","--ignore-user-config","--ignore-rules","--sandbox","read-only","--model",model,...off.flatMap(value=>["-c",`features.${value}=false`]),"-c","project_doc_max_bytes=0","-c",'web_search="disabled"',`${prompt}\nReturn one JSON object matching this schema: ${JSON.stringify(SCHEMA)}`];}
...
if(isMainModule(import.meta.url)){const h=/^--host=(claude|codex)$/.exec(process.argv.slice(2))?.[1];let text="";for await(const chunk of process.stdin){text+=chunk;if(Buffer.byteLength(text)>64*1024){text="";break;}}const input=text?JSON.parse(text):{},run=h==="codex"?runCodexObserver:h==="claude"?runClaudeObserver:null;if(run)run(input).then(value=>{if(value)process.stdout.write(`${JSON.stringify(value)}\n`);}).catch(()=>{});}
