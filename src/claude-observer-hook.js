...
export function claudeObserverEnvironment(environment){const result={...environment};for(const key of ["CLAUDECODE","CLAUDE_CODE_ENTRYPOINT","ANTHROPIC_API_KEY","ANTHROPIC_AUTH_TOKEN","ANTHROPIC_BASE_URL"])delete result[key];return result;}
...
export function claudeObserverArguments(model,prompt){return ["--bare","-p","--model",model,"--tools","","--disallowedTools","mcp__*","--permission-mode","dontAsk","--permission-prompts","none","--no-session-persistence","--max-turns","1","--output-format","json","--json-schema",JSON.stringify(SCHEMA),prompt];}
