...
export function callObserverCommand(command,args,cwd,env,timeout=30000){return new Promise((resolve,reject)=>execFile(command,args,{cwd,env,timeout,windowsHide:true,maxBuffer:262144},(error,stdout)=>error?reject(error):resolve({stdout,args})));}
function callClaude({cwd,model,prompt,environment}){const args=claudeObserverArguments(model,prompt);return callObserverCommand("claude",args,cwd,claudeObserverEnvironment(environment));}
...
function callCodex({cwd,model,prompt,environment}){const args=codexObserverArguments(model,prompt);return callObserverCommand("codex",args,cwd,codexObserverEnvironment(environment));}
