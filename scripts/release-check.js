...
export async function releaseCheck(options) {
  const [pkg, lock, claude, codex, marketplace, changelog] = await Promise.all([
    ...
    json(options.root, ".claude-plugin/marketplace.json"),
    readFile(resolve(options.root, "CHANGELOG.md"), "utf8")
  ]);
  assert(SEMVER_RE.test(pkg.version || ""), "package version is not valid SemVer");
  const versions = [lock.version, lock.packages?.[""]?.version, claude.version, codex.version, marketplace.plugins?.[0]?.version];
