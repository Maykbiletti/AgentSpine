...
// Includes bounded provider-handoff validation, provenance and its documented contract.
const MAX_PACKED_BYTES = 524 * 1024;
const MAX_UNPACKED_BYTES = 2344 * 1024;
...
function validatePackageReport(report, version) {
  ...
  assert(Number.isInteger(item.size) && item.size > 0 && item.size <= MAX_PACKED_BYTES, "npm package exceeds the 524 KiB packed release limit");
  assert(Number.isInteger(item.unpackedSize) && item.unpackedSize > 0 && item.unpackedSize <= MAX_UNPACKED_BYTES, "npm package exceeds the 2344 KiB unpacked release limit");
