...
// Includes the measured artifact evaluator, CLI wiring and its shipped contract.
// Bounded growth: +1 KiB compressed / +16 KiB source; no dependencies added.
const MAX_PACKED_BYTES = 525 * 1024;
const MAX_UNPACKED_BYTES = 2360 * 1024;
...
function validatePackageReport(report, version) {
  ...
  assert(Number.isInteger(item.size) && item.size > 0 && item.size <= MAX_PACKED_BYTES, "npm package exceeds the 525 KiB packed release limit");
  assert(Number.isInteger(item.unpackedSize) && item.unpackedSize > 0 && item.unpackedSize <= MAX_UNPACKED_BYTES, "npm package exceeds the 2360 KiB unpacked release limit");
