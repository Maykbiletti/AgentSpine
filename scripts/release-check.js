...
const MAX_PACKED_BYTES = 520 * 1024;
// Includes the native Codex history adapter and its documented format contract.
const MAX_UNPACKED_BYTES = 2336 * 1024;
...
function validatePackageReport(report, version) {
  ...
  assert(Number.isInteger(item.size) && item.size > 0 && item.size <= MAX_PACKED_BYTES, "npm package exceeds the 520 KiB packed release limit");
  assert(Number.isInteger(item.unpackedSize) && item.unpackedSize > 0 && item.unpackedSize <= MAX_UNPACKED_BYTES, "npm package exceeds the 2336 KiB unpacked release limit");
