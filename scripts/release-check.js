...
// Includes the artifact evaluator and exact-task action-time learning context.
// Current bounded growth: +2 KiB packed / +3 KiB unpacked; no dependencies added.
const MAX_PACKED_BYTES = 527 * 1024;
const MAX_UNPACKED_BYTES = 2363 * 1024;
...
function validatePackageReport(report, version) {
  ...
  assert(Number.isInteger(item.size) && item.size > 0 && item.size <= MAX_PACKED_BYTES, "npm package exceeds the 527 KiB packed release limit");
  assert(Number.isInteger(item.unpackedSize) && item.unpackedSize > 0 && item.unpackedSize <= MAX_UNPACKED_BYTES, "npm package exceeds the 2363 KiB unpacked release limit");
