...
function validateHooks(root, hooks, { required, commandRoot, matcher = TIMELINE_PRE_TOOL_MATCHER, asyncObserver = false }) {
...
assert(Array.isArray(registrations[0].hooks) && registrations[0].hooks.length === expectedCount, `${event} has an unsafe hook command count`);
...
return { events: required, commands: required.length+(asyncObserver?1:0), entrypoint: relative(root, resolve(root, "src/hook.js")) };
}
