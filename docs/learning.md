...
`behavior` candidates use content-free `agentspine.learning-outcome/v9` receipts while existing v1-v8 history remains readable. Before any new baseline is recorded, a local operator must register each evaluator principal root and then one immutable `agentspine.learning-evaluation/v26` contract. The contract fixes SHA-256 digests for the exact evidence-backed candidate revision, task, dataset and evaluator protocol, the exact persona/user/tenant/project/group/task scope, the metric and direction, eligible evaluator IDs, one distinct locally confirmed SHA-256 principal root per evaluator, minimum case count, expiry, candidate-confidence and independent-evidence gates, the fresh candidate-evidence cohort and age policy, an explicit-user-or-objective-test source quorum, promotion and regression thresholds, same-root pairing, the initial measurement trials, their completion and staleness policies, and the rule that any eligible blocking defect overrides numeric averages.
...
Version 0.45 freezes the remaining candidate-admission gates before measurement. New initial contracts use `agentspine.learning-evaluation/v16`; bounded corrective contracts use v17. Both store only the numeric `minConfidence` and `minEvidence` thresholds already active at local registration. Automatic promotion reads those frozen values, and retry integrity checks use the retry contract's stored evidence floor rather than mutable project configuration. Lowering the defaults therefore cannot admit a candidate that failed its original contract, while raising them cannot make a valid registered experiment unreadable after restart. Historical v1-v15 contracts retain their prior behavior. Rewriting a stored gate fails closed; exact-scope status and Context MCP expose only the matching contract count and a content-free gate digest, while Doctor and audit expose project-wide counts.

Version 0.46 makes those gates a prerequisite rather than a later promotion check. New initial contracts use `agentspine.learning-evaluation/v18`; bounded corrective contracts use v19. Each embeds a digested `agentspine.learning-candidate-admission/v1` with only the exact target and scope digests, frozen numeric gates, observed confidence, distinct evidence count, admission time, decision and context-only authority. A candidate below either gate creates no evaluation contract, so no measurement can be attached to a knowingly ineligible experiment. The evidence floor is always at least two for behavior evaluation. Six concurrent registrations converge on one contract; re-signing a false count, target, scope or threshold fails closed after restart. Historical v1-v17 contracts remain readable. Scoped CLI and Context MCP return only matching counts and an active admission digest; foreign scopes return zero, while Doctor and audit remain project-wide.

Version 0.47 makes that admission proof temporally and evidentially exact. New initial contracts use `agentspine.learning-evaluation/v20`; bounded corrective contracts use v21. Their `agentspine.learning-candidate-admission/v2` receipts include a digested age policy and an ordered, content-free cohort containing only each eligible evidence digest, independence digest, evidence class and observation time. Confidence and the independent-evidence count are computed from that fresh cohort rather than stale history. Evidence older than the frozen `outcomeMaxAgeDays` window is ineligible; any future-dated observation rejects registration. Later configuration changes affect only future contracts. Re-signing a changed cohort, timestamp, independence digest or age policy fails closed after restart. Historical v1-v19 contracts remain readable. Exact-scope status and Context MCP expose only the matching cohort-contract count and active policy/cohort digests; foreign scopes return zero, while Doctor and audit expose project-wide counts.

Version 0.48 binds the no-averaging exception before any baseline result exists. New initial contracts use `agentspine.learning-evaluation/v22`; bounded corrective contracts use v23. Their content-free `agentspine.learning-blocking-defect-policy/v1` fixes the covered Before, After and revalidation phases, `any-defect-overrides-average` aggregation, pre-Canary rejection and automatic rollback. One eligible Before receipt with a blocking defect therefore prevents Canary admission even if two 0.99 scores would otherwise pass, and the precommitted evaluator/run slot cannot be replaced by a favorable replay. Retry comparison v3 binds the same policy digest across a new v22 failure and its v23 correction. Historical v1-v21 contracts remain readable, including bounded retries from earlier versions. Re-signing a weakened phase, aggregation or action fails closed after restart. Exact-scope status and Context MCP expose only the matching contract and blocking-receipt counts plus the active policy digest; foreign scopes return zero, while Doctor and audit expose project-wide counts.

Version 0.49 makes evidence provenance a prerequisite rather than treating any two independent observations as sufficient. New initial contracts use `agentspine.learning-evaluation/v24`; bounded corrective contracts use v25. Their content-free `agentspine.learning-evidence-source-policy/v1` requires at least one fresh independent `user-statement` or `test` anchor in the already frozen evidence cohort. Interaction-only and document-only cohorts therefore create no evaluation contract and cannot receive measurements, even if their confidence and evidence count pass. Retry comparison v4 binds the same source-policy digest across a new v24 failure and its v25 correction. Historical v1-v23 contracts remain readable. Re-signing qualifying classes or the minimum quorum fails closed after restart. Exact-scope status and Context MCP expose only the matching contract count and active policy digest; foreign scopes return zero, while Doctor and audit expose project-wide counts.

Version 0.50 makes those qualifying labels locally accountable. New initial contracts use `agentspine.learning-evaluation/v26`; bounded corrective contracts use v27. Registration requires `--confirm-local-evidence-sources` and embeds ordered `agentspine.learning-evidence-source-attestation/v1` records in candidate admission v3. Each record binds only the evidence and independence digests, the confirmed source class and time; it stores no evidence ID, summary, transcript or source content. A self-labeled `user-statement` or `test` creates no contract without the explicit local confirmation. Re-signing a false class fails closed against the frozen candidate cohort after restart. Historical v1-v25 contracts remain readable. Exact-scope status and Context MCP expose only matching attested-contract counts and digests; foreign scopes return zero, while Doctor and audit expose project-wide counts.

Version 0.51 makes that local confirmation precisely withdrawable without rewriting the contract or its evidence. `learn-evidence-source-attestation-revoke` requires `--confirm-local-evidence-source-attestation-revocation` and records one `agentspine.learning-evidence-source-attestation-revocation/v1` receipt binding the evaluation, candidate admission, evidence and independence digests, source class, target and exact scope. The explanation is retained only as a digest. A dependent active or validated lesson disappears from preflight context immediately; measurements, projections, deliveries, outcomes and revalidation fail closed, and the next locked evaluation pass rolls the lineage back and restores a safe predecessor. Concurrent identical withdrawals are idempotent, conflicting replay and re-signed bindings fail closed after restart, exact-scope diagnostics expose only matching counts and digests, and foreign scopes receive zero.
...
agentspine learn-evaluation evaluation:check-invariant-v1 \
  ...
  --persona agent:synthetic --user user:synthetic --tenant tenant:synthetic \
  --project project:synthetic --task task:synthetic \
  --confirm-local-evaluation --confirm-local-evidence-sources
...
agentspine learn-evaluation evaluation:check-invariant-retry-v1 \
  ...
  --confirm-local-trial-retry --confirm-local-evaluation --confirm-local-evidence-sources
