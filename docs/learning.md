...
`behavior` candidates use content-free `agentspine.learning-outcome/v5` receipts while existing v1-v4 history remains readable. Before any new baseline is recorded, a local operator must register one immutable `agentspine.learning-evaluation/v2` contract. The contract fixes SHA-256 digests for the task, dataset and evaluator protocol, the exact persona/user/tenant/project/group/task scope, the metric and direction, eligible evaluator IDs, minimum case count, expiry, and the current promotion and regression thresholds. Each new outcome repeats the exact registered dataset digest and a measured case count at or above that floor. The receipt stores no cases, task, dataset, prompt, answer, transcript, credential, or source content. Metric values are normalized to `0..1`; objective measurements, explicit user feedback, and model suggestions remain separate. Model suggestions are retained for diagnosis but never count toward automatic promotion or validation. Legacy v1 evaluation contracts remain readable and reproducible, but only v2 contracts and v5 outcomes prove minimum case coverage.
