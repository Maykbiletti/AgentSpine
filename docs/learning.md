...
`behavior` candidates use content-free `agentspine.learning-outcome/v8` receipts while existing v1-v7 history remains readable. Before any new baseline is recorded, a local operator must register one immutable `agentspine.learning-evaluation/v5` contract. The contract fixes SHA-256 digests for the task, dataset and evaluator protocol, the exact persona/user/tenant/project/group/task scope, the metric and direction, eligible evaluator IDs, minimum case count, expiry, promotion and regression thresholds, and same-evaluator pairing.

Version 0.18 keeps the separate immutable `agentspine.learning-measurement/v1` registration boundary and adds paired evaluator cohorts. Every counted after-outcome must use an evaluator that produced one frozen before-outcome under the same contract, measurement kind and case count. Only one outcome per evaluator and phase counts, and every after-outcome binds a different completed model turn. The Canary score is the mean of evaluator-local deltas, not the difference between two independently weighted cohort averages. Repeated runs therefore cannot overweight one evaluator, evaluator or measurement-kind substitution cannot simulate improvement, and two evaluations of one response cannot simulate two applications.

Measurement receipts still bind the exact contract, phase, scope, metric value, blocking-defect count, measurement kind, evaluator ID, stable provider run ID, source digest, dataset coverage and measurement time. They store no cases, task, dataset, prompt, answer, transcript, credential, or source content. Source digests and evaluator/run pairs remain globally single-use across the project. `learn-outcome` consumes exactly one receipt; copied metric, scope or coverage fields cannot substitute for its immutable digest, and a receipt cannot be consumed twice. Objective measurements, explicit user feedback and model suggestions remain separate. Model suggestions never count toward automatic promotion or validation. Legacy v1-v4 evaluation contracts and v1-v7 outcome receipts remain readable; only v5/v8 contracts prove paired cohort lineage.
...
An unplanned, stale, cross-scope, metric-drifted, evaluator-drifted, dataset-drifted, under-covered, provenance-free, run-replayed or source-replayed measurement is rejected, as is an unbound or forged application or delivery ID. Independent receipts meeting the frozen contract threshold validate the lesson only when they reference distinct globally registered measurement runs, the same before/after evaluator cohort and distinct completed turns. A second contract, repeated runs by one evaluator, evaluator substitution, two evaluator names around one result or two evaluators of one turn cannot simulate independence. Later configuration changes cannot weaken an already registered experiment. Any blocking defect rolls it back immediately; any paired regression beyond the frozen tolerance, insufficient mean paired improvement, or contract/Canary expiry before validation also rolls it back. A superseded lesson is restored atomically. No average score can override a blocking defect.
