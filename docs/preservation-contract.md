...
- writes catalogs, graph overlays, attention, learning, delegation policy, coordination, and imported sharing state only to the user state directory;
...
Uninstall removes the plugin and its generated state only. It never touches scanned projects. Acceptance tests snapshot source bytes before scanning, resolving, reading, attention mutation, learning proposal/review/rollback, delegation and task workflows, shared adapter publication/import/review, verifying, and hook execution, then compare the source tree afterward.
