...
AgentSpine `0.5.0` includes one visible, reproducible acceptance scenario for the complete automatic lifecycle. It runs the production Claude Code and Codex hook adapter directly, never selects an MCP tool, and prints a receipt for every verified boundary.
...
`npm run host:install-check` stages both a fresh installation and an upgrade from `0.4.0`. Each installed `0.5.0` bundle must expose exactly one MCP server and one hook set, then pass the complete visible acceptance run with `mcpCalls: 0`. Uninstall removes only staged plugin and generated state; the synthetic source hashes must remain unchanged.
