...
flowchart TB
    ...
    subgraph Core["AgentSpine core"]
      ...
      G["Context-only graph + world model + attention + learning + continuity + tasks + shared quarantine"]
...
sequenceDiagram
    ...
    B->>C: native sources, relationships, tasks, world facts, accepted learning, reviewed sharing
  ...
  projects/
    ...
    signers/
      ...
      private/
        <key-fingerprint>.pem
  integrity/
    session-timeline-signing.key
    session-timeline-state-<root-digest>.json
    session-timeline-head-<root-digest>.json
    session-timeline-enrollment-<root-digest>.json
    session-timeline-invocations-<root-digest>.json
```

`source-roots.json` retains only explicit or host-evidenced source/state bindings, profile digests, provenance, rollback, and purge history; it never stores source content or authority. `indexed-memory-cache.json` is a bounded private cache of integrity-checked indexed-memory snapshots and file identities. It is invalidated by index changes, correction, deletion, binding rollback, or purge; it remains context-only and cannot grant identity, rights, trust, or execution. `lesson-recall-sessions.json` stores a short scope-bound list of selected memory source digests, not lesson text. `session-timeline-enrollment-<root-digest>.json` stores only explicit local-owner, expiring private bindings and source anchors; no normal hook field can substitute for it. Integrity-private `session-timeline-state-<root-digest>.json` sidecars store an authenticated, bounded index of redacted objective-event cards and transcript metadata, never transcript bytes; a signed latest-state head rejects sidecar-only rollback while the protected integrity anchor remains intact, and a source, head, or signature mismatch yields no history. One-use invocation records bind a real host tool use to its exact MCP request and are consumed atomically. `catalog.json` is reproducible provenance. `graph.json` stores reversible annotations, relationships, privacy scopes, confidence, and superseded observations. `world-model.json` keeps append-only measured, explicitly user-confirmed, and model-proposed assertions separate; expiry and disagreement remove claims from facts and expose uncertainty. `attention.json` stores bounded follow-up cues, minimal interaction timestamps, quiet-hour policy, presentation throttles, and hook-driven heartbeat, promise, and blocker lifecycles with idempotent receipts and retained prior values. `learning.json` separates evidence-backed candidates from accepted context and records review, promotion, supersession, rollback, a locally confirmed revocable evaluator-root registry, immutable registry-to-evaluation bindings, digest-only evaluation contracts, content-free dataset/case coverage, evaluator-root independence, globally unique measurement-run lineage, locally confirmed digest-only measurement, application, delivery and outcome revocations, root-paired before/after evaluator cohorts, distinct turn projections and deliveries, single-use contract-bound outcomes, and bounded Canary history. `continuity.json` stores only opt-in configuration and minimal signal receipts with prompt digests, never transcripts. `coordination.json` stores only context tasks, open threads, handoffs, and prior versions. `gateway-policy.json` stores authenticated goals, dependency-bound plans, durable context-only knowledge gaps, locally confirmed resolutions, and content-free self-help research provenance; `gateway-runtime.json` stores only leased execution, delivery and diagnostic state. `delegation-policy.json` is physically separate and contains only explicit local task-coordination grants. `execution-policy.json` contains exact locally confirmed self-starter grants; `selfstarter.json` contains leased jobs, content-bound checkpoints, retry state, retained prior versions, and idempotent receipts. Neither is context authority, and neither is writable through MCP. `sharing.json` quarantines imports and retains local review, supersession, rollback, and signature proof. `sharing-trust.json` is a project-local allowlist of public signing keys; the installation-wide signer registry keeps private keys separate. Policy, trust, keys, and adapter administration are not writable through MCP. All are private user state. This gives uninstall a simple, auditable property: removing AgentSpine state cannot remove or alter original agent files.

For the session-timeline adapter, the enrollment file begins with a pending opaque host receipt created only after verified host preflight and becomes an explicit locally confirmed immutable snapshot. It stores no transcript content. Lifecycle status is source-blind; only receipt consumption and a matching `PreToolUse` may revalidate bounded source metadata and a fixed prefix. Historic reading is reserved for the exact one-use MCP index, search or capture invocation. Capture reopens one selected raw line and writes only mechanically derived objective fields and provenance into the existing world model. A separate strict native-user event may replace only the next step of an existing same-route continuation when the entire message uses an explicit correction prefix; active-set compare-and-swap retains one current checkpoint under races. Neither path adds a second memory store or accepts model claims as evidence. The state/head pair detects state-only and mixed rollback and permits only an exact one-generation torn-write repair under the owned lock; restoring a complete matching local integrity directory remains outside its local detection boundary.
