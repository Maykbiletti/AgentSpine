...
Hook payloads can contain images or text larger than the adapter's 64 KiB memory budget. The shipped Claude, Codex and King registrations bind `SessionStart`, `UserPromptSubmit`, `PreCompact` and `PostCompact` with `--context-event=<event>`. At overflow, the adapter drains stdin without parsing or storing a prefix, exits successfully and emits a bounded `loaded: false` / `hook-input-budget` diagnostic. Recall and capture are unverified for that event; the host can continue with its original input under native rules. Existing memories and user files remain untouched. This is a degraded fallback, not full recall for oversized requests or image extraction. Small complete inputs still take the normal recall path.

The event binding comes from the installed command, never a guessed JSON prefix. A parsed event must match it. PreToolUse and stop registrations retain their existing checks; PostToolUse retains its separate silent-oversize lane. Malformed small JSON is still rejected. Update the hook registrations together with the code: copying only `src/hook.js` does not activate the fallback. Native host trust and installation requirements continue to apply.
