...
Version `0.70.0` replaces the `0.69.0` plugin cache identity and splits the rights-bound self-starter into bounded security domains while preserving its public surface and stored contracts. Writing deliveries must call `session_briefing`, `delivery_knowledge_query` and `record_delivery_premortem` in that order with the exact hook-issued requirement before their first mutation.
