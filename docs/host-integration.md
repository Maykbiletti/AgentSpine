...
Version `0.72.6` replaces the `0.72.5` cache identity and adds an explicit Codex registration command. Version `0.72.5` accepts a delivery test only from its structured zero process exit or command-bound final marker; transport success alone is insufficient. A prompt without an assignment identifier starts a new durable delivery. For an unfinished delivery, the host may carry the exact `assignment_id` (or `assignmentId`) from the prior hook response in `UserPromptSubmit`; all scope fields must still match. This reuses the existing requirement without rewriting history or clearing open write/test obligations. Closed, foreign or unknown assignments cannot be continued. Prompt wording alone never selects continuation. See [the exact contract and limits](assignment-continuation.md). Writing deliveries must call `session_briefing`, `delivery_knowledge_query` and `record_delivery_premortem` in that order with the exact hook-issued requirement before their first mutation.
...
For a direct npm/package installation, register or update the MCP reader explicitly. The command writes only a marked AgentSpine block, refuses an unmanaged conflicting registration, and requires a local confirmation flag:

```bash
agentspine host-install codex --confirm-local-host-install --json
```

Use `--codex-home /absolute/profile` only for a non-default or synthetic profile. The stable launcher lives below that profile and points through an atomically replaced, digest-sealed registration; `config.toml` does not need a versioned cache path. Start a new Codex process after a successful update. During MCP `initialize`, the launcher checks the canonical package root, exact package/runtime version and required tools before it forwards any state-bearing request. A failed check returns no tools and does not migrate or delete state. User-controlled executable and hook trust remain separate and mandatory. The configuration shape follows the official [Codex MCP configuration contract](https://learn.chatgpt.com/docs/extend/mcp?surface=cli).
