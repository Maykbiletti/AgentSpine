...
Version `0.71.0` replaces the `0.70.0` plugin cache identity and adds provenance-bound world context to the provider-neutral MCP and session briefing while preserving all execution boundaries. Writing deliveries must call `session_briefing`, `delivery_knowledge_query` and `record_delivery_premortem` in that order with the exact hook-issued requirement before their first mutation.
