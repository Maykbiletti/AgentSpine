...
Tests feed the real hook a structured child-process exit result. They do not claim to reproduce the reported native Codex/PowerShell envelope. Native tools discovery, automatic MCP registration and loaded-reader checks before migration still require implementation and host evidence. Version 0.72.4 adds [structured MCP completion for ordinary assignments](structured-completion.md); native-host activation remains unproved. CodexLink is outside this repository change.
