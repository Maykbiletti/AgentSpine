...
Before opening an objective-observation gap, a goal-plan host receives `selfHelpPolicy`. It directs the host to inspect the current repository first and to use public primary sources only when repository evidence is insufficient. A successful runner result uses `selfHelp` instead of `knowledgeGap`: it contains 1-16 repository references with exact commit and SHA-256 provenance, and either marks that evidence sufficient or adds 2-8 fresh sources from at least two independent public HTTPS origins. External references retain URL, kind, version, license, digest, observation time and relevance, but never fetched content.

AgentSpine validates the order, freshness, scope, source classes and digests, marks all external evidence untrusted, resolves the objective gap under the authenticated agent identity, and queues exactly one continuation without asking the user. Restart reconciliation recreates one lost continuation. Owner questions remain appropriate for choices only the owner can make; missing network access is a visible blocker rather than invented evidence. Self-help reports are context only and cannot grant tools, permissions, identity, delegation or policy exceptions.

When two independent public primary sources remain incompatible after the repository-first search, `selfHelp` may instead return `outcome: "needs-owner-input"`. The escalation must bind exactly two conflicting source digests from different origins, a bounded decision reason, one question and 2-8 distinct options. AgentSpine then stores the content-free evidence, opens one owner-input gap and stops the exact step. A missing, duplicated or fabricated conflict cannot create a question; the locally confirmed `goal-clarify` answer resumes the plan once, including after restart.
