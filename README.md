...
Version `0.73.0` adds bounded session evidence without copying transcripts. Separate Claude, Codex, and King adapters share one private scope-bound index. Verified hooks may add a small tail only after every old byte passes its digest check. One-use permits block raw MCP, replay, scope changes, and altered sources; cross-provider recall stays opt-in. Captured measurements and strict native corrections remain context-only and grant no authority. See [bounded session timeline](docs/session-timeline.md).
