...
Version `0.23.0` retains the `agentspine.preflight/v2` pre-answer contract, self-healing authenticated persona-to-graph reconciliation, and provider-neutral outcome-bound behavior loop. Every new automatic behavior experiment starts with an immutable, locally confirmed evaluation contract. Evaluator roots must first enter a separate local append-only registry; an immutable binding receipt connects the exact active registry records to evaluation v7. Root aliases, reuse and unconfirmed roots are rejected. Revocation immediately prevents new measurements and outcomes, removes an affected active or validated lesson from preflight context, and causes the next evaluation pass to roll it back. Each successful v7 validation creates a content-free evidence lease binding the exact contract, registry binding, scope, metric, before/after outcome digests, measured improvement and expiry. Missing, stale, altered or evaluator-revoked leases fail closed before the next model turn. Renewal precommits the first completed exact-scope turns and assigns frozen evaluator roots to deterministic slots before results exist; later high-scoring turns cannot replace earlier failures. A separate content-free measurement receipt binds the exact contract, scope, dataset coverage, metric, evaluator, stable run ID, timestamp and SHA-256 source identity before an outcome may consume it. Source identities and evaluator/run pairs are globally single-use across the project. Independence, run replay protection and same-evaluator before/after pairing use the principal root, so two names for one evaluator cannot manufacture two votes. One outcome per evaluator root and phase may count, each after-result requires a different completed model turn, and improvement is the mean of paired deltas. Each active Canary receives a content-free projection receipt after the exact hard preflight is consumed, then a separate exact-session delivery receipt only when the native host reports `Stop` or `SubagentStop`. A crash before model completion cannot count as learning. Stale projections and unconsumed measurements are bounded, diagnosed and locally purgeable. The installed hook reuses one bounded source catalog through scope, continuity, learning, attention, persona, relationship and briefing reads. Canonical Windows home identities are excluded from recursive project enumeration even when the home contains a project marker. When the working root is exactly the user home, the configured AgentSpine state subtree is an explicit scanner exclusion, so authenticated state remains usable without entering context; state inside an ordinary nested project still fails closed. Direct host-native rules remain fully injected and byte-preserved. Exact roster groups remain context-only and isolated; model suggestions never count as promotion evidence; blocking defects and paired regressions still roll back Canaries. Claude keeps an 8 KiB instruction standard with one signed, revalidated overflow up to 16 KiB. Claude uses `hooks/hooks.json`; Codex ships the separate validated `hooks/codex.json` adapter. Fresh-install and upgrade checks prove package containment and entrypoint behavior; actual hook discovery and trust remain host-controlled and must be inspected in the live host. See [safe learning](docs/learning.md), [pre-answer recall gate](docs/preflight-recall.md), and [durable gateway worker](docs/gateway-runtime.md).
