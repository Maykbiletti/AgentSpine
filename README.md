...
Version `0.10.1` retains the `agentspine.preflight/v2` pre-answer contract and self-healing authenticated persona-to-graph reconciliation. Exact roster groups are materialized as context-only entities, stale membership edges are repaired, inactive personas disappear from current relationship reads, and exact group briefings exclude cross-group edges. Relationship reads bypass project scans and degrade visibly after an aborting five-second deadline. An unmarked home-directory working directory never triggers a recursive home or Dropbox scan. Claude keeps an 8 KiB instruction standard with one signed, revalidated overflow up to 16 KiB; larger files block immediately with measured byte counts. Claude uses `hooks/hooks.json`; Codex selects `hooks/codex.json` so Claude-only events never enter Codex configuration. Fresh-install and upgrade checks prove package containment and entrypoint behavior; actual hook discovery and trust remain host-controlled and must be inspected in the live host. See [pre-answer recall gate](docs/preflight-recall.md) and [durable gateway worker](docs/gateway-runtime.md).
