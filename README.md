...
Version `0.24.0` retains the `agentspine.preflight/v2` pre-answer contract, self-healing authenticated persona-to-graph reconciliation, and provider-neutral outcome-bound behavior loop. Every new automatic behavior experiment starts with an immutable, locally confirmed evaluation contract. Evaluator roots first enter a separate local append-only registry; an immutable binding receipt connects the exact active registry records to evaluation v7. Root aliases, reuse and unconfirmed roots are rejected. Revocation immediately prevents new measurements and outcomes, removes an affected active or validated lesson from preflight context, and causes the next evaluation pass to roll it back. Each successful v7 validation creates a content-free evidence lease binding the exact contract, registry binding, scope, metric, outcome digests, measured improvement and expiry. Missing, stale, altered or evaluator-revoked leases fail closed before the next model turn. Renewal atomically admits the first exact-scope projections and assigns frozen evaluator roots before results exist. A missing `Stop`, crash or intentional delay leaves its admitted slot incomplete; later high-scoring turns cannot replace it or extend the lease. Measurement receipts bind contract, scope, dataset coverage, metric, evaluator, run, timestamp and SHA-256 source identity before an outcome may consume them. Source identities and evaluator/run pairs are globally single-use. Independence and same-evaluator before/after pairing use the principal root, so aliases cannot manufacture votes. One outcome per root and phase may count, every after-result requires a distinct completed model turn, and no average can hide a blocking defect. The installed hook reuses one bounded source catalog through scope, continuity, learning, attention, persona, relationship and briefing reads. Canonical Windows home identities are excluded from recursive project enumeration even when the home contains a project marker. At the exact user home, configured AgentSpine state is explicitly excluded from scanning; state inside an ordinary project remains fail-closed. Host-native rules stay fully injected and byte-preserved. Exact roster groups remain context-only and isolated, and model suggestions never count as promotion evidence. Claude keeps an 8 KiB instruction standard with one signed, revalidated overflow up to 16 KiB. Claude uses `hooks/hooks.json`; Codex ships `hooks/codex.json`. Fresh-install and upgrade checks prove package containment and entrypoint behavior; actual hook discovery and trust remain host-controlled and require live-host verification. See [safe learning](docs/learning.md), [pre-answer recall gate](docs/preflight-recall.md), and [durable gateway worker](docs/gateway-runtime.md).
...
| `agentspine learn-evaluation …` | Register an immutable, digest-only fixed-task evaluation contract locally |
...
Safe learning is evidence-first: general candidates remain invisible until reviewed. Low-risk behavior candidates additionally require a locally confirmed immutable evaluation contract and independent fixed-task measurements before and after an exact-scope Canary. After-results count only when they bind to that unchanged contract, distinct preflight-bound projection receipts and exact-session model-stop delivery receipts; model self-evaluation cannot promote them, later config cannot lower frozen thresholds, and regressions roll back automatically. A separate default-off continuity opt-in can automatically accept only direct, high-confidence style, preference, no-go, correction, project-fact, and reference signals. Sensitive personal facts, secrets, identity merges, private group content, and operational or authority claims are always rejected. See [automatic continuity](docs/automatic-continuity.md) and [safe learning](docs/learning.md).
